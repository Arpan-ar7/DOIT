...
import { CATEGORY_EMOJIS, RequestCategory, EXPIRY_OPTIONS, DEFAULT_EXPIRY_HOURS, DEFAULT_DELIVERY_FEE } from '../../constants/mockData';
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <Text style={styles.helper}>Tell campus mates what you need. Only verified students can see your request.</Text>

          <View style={styles.formCard}>
            <Text style={styles.label}>Category</Text>
            <View style={styles.categoryRow}>
              {CATEGORY_EMOJIS.map((c) => (
                <Pressable key={c.category} style={[styles.categoryOption, category === c.category && styles.categoryOptionActive]} onPress={() => setCategory(c.category)}>
                  <Text style={{ fontSize: 20 }}>{c.emoji}</Text>
                  <Text style={[styles.categoryOptionLabel, category === c.category && styles.categoryOptionLabelActive]}>{c.label}</Text>
                </Pressable>
              ))}
            </View>
            ...
            <View style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={styles.input} keyboardType="number-pad" value={deliveryFee} onChangeText={setDeliveryFee} />
              </View>
            </View>
            ...
            {showFeeHint && (
              <View style={styles.feeHintBox}>
                ...
                <Text style={styles.feeHintText}>A higher delivery fee usually gets your request accepted faster.</Text>
              </View>
            )}
            ...
            <View style={styles.expiryChipRow}>
              {EXPIRY_OPTIONS.map((opt) => (
                <Pressable key={opt.hours} style={[styles.expiryChip, expiryHours === opt.hours && styles.expiryChipActive]} onPress={() => setExpiryHours(opt.hours)}>
                  <Text style={[styles.expiryChipText, expiryHours === opt.hours && styles.expiryChipTextActive]}>{opt.label}</Text>
                </Pressable>
              ))}
            </View>
            ...
            <View style={styles.expiryBox}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.expirySub}>Your request will automatically close after that.</Text>
              </View>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  categoryOption: { width: 66, paddingVertical: 10, borderRadius: 12, backgroundColor: '#fafbf8', borderWidth: 1, borderColor: '#e2e7e0', alignItems: 'center', gap: 4 },
